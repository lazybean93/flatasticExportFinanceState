...
cat Flatastic\ _\ Web\ App_files/saved_resource.html | sed 's/ <\/span> <span style="float:right;white-space:nowrap" class="ng-binding"> /\n/g' | sed 's/ <\/span> <\/div>/\n/g' | sed 's/Keine weiteren Einträge vorhanden/\n\n/g' | grep -v '<\|=\|\/\|^$\|’\|,' > dates.tmp
...
cat Flatastic\ _\ Web\ App_files/saved_resource.html | sed 's/<\/span> <div class="ft-card__header"> <span class="ng-binding"> /\n/g' | sed 's/ <\/span> <span style="float:right;white-space:nowrap" class="ng-binding">/\n/g' | sed 's/Keine weiteren Einträge vorhanden/\n\n/g' | grep -v '[0-9]\|"\| \|^$' > names.tmp
